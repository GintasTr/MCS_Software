...
def scanning_procedure(object_coords, colour_data, object_area):
    while True:
        ...
        if foreign_object == "No blobs found":                          # If no blobs were found
            state = "Not found"
            text2 = "Object is: %s" % state
            img.dl().text(text2, (10, 10),color=Color.RED) #FOR FEEDBACK ONLY
            img.show()  #FOR FEEDBACK ONLY
            continue
        state = "Found"
        text2 = "Object is: %s" % (state)
        foreign_object.drawMinRect(layer=img.dl(),color = Color.RED, width = 3)
        img.dl().setFontSize(25)
        img.dl().text(text2, (foreign_object.bottomLeftCorner()[0] + 10, foreign_object.bottomLeftCorner()[1] + 10 ),

...
def read_comms_output(I2C_DEVICE_ADDR):
    ...
    # Return dictionary in format of comms_output ['fault_bit'] ['stop_bit']
    return comms_output


# Function to decode comms raw output which is a string in format of #$F(fault_bit)S(stop_bit)%
def interpret_comms_output(comms_output_raw):
    fault_bit = int(comms_output_raw[3])                             # Read the fault bit
    stop_bit = int(comms_output_raw[5])                              # Read the stop bit
                                                                # Encode the output in to a dictionary
    comms_output = {"fault_bit": fault_bit, "stop_bit": stop_bit}

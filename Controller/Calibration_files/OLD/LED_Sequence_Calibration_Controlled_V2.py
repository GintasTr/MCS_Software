...
def GetColourData(img, coords):
    crop_size = 25                                              # Area around the point to be evaluated (square width)
...
def perform_calibration():
    ...
    while not calibration_done:                                 # Loop while calibration is not done
        ...
        m_led_blob = LED_Sequence_Controlled_V5.MainLedDetection(m_led_img, m_led_coords, m_led_data)
        ...
        if scan_type == "illumination_level":                   # If scan is based on illumination_level
            ...
            while(elapsed_time<seq_time):                           # Loop while sequence is not finished
                ...
                area_light = LED_Sequence_Controlled_V5.GetLight(live_img, m_led_coords, m_led_data, dist_led)

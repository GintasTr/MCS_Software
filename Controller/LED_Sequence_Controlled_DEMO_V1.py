...
def BlobsNumber(img, coords, hsv_data, dist):
    ...
    while True:
        ...
        if current_state == old_state:
            ...
            text = "LED is: %s for: %.1fs" % (current_state, elapsed_time1)
        if current_state != old_state:
        ...
        img.dl().centeredRectangle((blob_coordinates[0],
                            blob_coordinates[1]),(crop_length,crop_length), color = Color.RED, width = 3)
        img.dl().setFontSize(35)
        img.dl().text(text, (crop_length +10, 10),
                      color=Color.RED) #FOR FEEDBACK ONLY
        img.dl().sprite(cropped)

...
def GetImage():
    ...
    cv2.normalize(img, img, 0, 65535, cv2.NORM_MINMAX)      # Normalize image
    np.right_shift(img,8,img)                               # Shift to 8 bit array
    cv2.imwrite(STORED_IMAGE_NAME, np.uint8(img))           # Write the image to file

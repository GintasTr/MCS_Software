...
img = GetImage()
coords = GetClickCoords(img, "Click on the Handle")
colour_data = GetColourData(img, coords)
disp = Display()
while disp.isNotDone():
    img = GetImage()
    handle = MainLedDetection(img, coords, colour_data)
    ...
    angle=handle.angle()
    img.dl().text(str(angle), handle.bottomRightCorner(), color=Color.RED)
